...
//fraction の separatorにクラスを当てる
document.addEventListener('DOMContentLoaded', function () {
  const swiperPagination = document.querySelector('.swiper-pagination-fraction');
  if (swiperPagination) {
    const separator = swiperPagination.childNodes[1]; // インデックス1は / に対応
    if (separator && separator.nodeType === Node.TEXT_NODE) {
      const span = document.createElement('span');
      span.classList.add('swiper-pagination-separator');
      span.textContent = separator.textContent;
      swiperPagination.replaceChild(span, separator);
    }
  }
});
